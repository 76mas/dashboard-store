...
const VoucherSpace = () => {
  // const { setDeleteitems, setShowAlert } = useStatment();
  ...
  const getAllVoucher = async () => {
    try {
      let res = await axios.get("http://localhost:4000/voucher");
  ...
  const handelChangeActive = async (id, newActive) => {
    ...
    try {
      await axios.put(`http://localhost:4000/voucher/${id}`, {
        active: newActive,
      });
      getAllVoucher();
    } catch (e) {
...
const AlertDelete = ({ id, setShowAlert, refresh, setRefresh }) => {
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:4000/voucher/${id}`);
      setShowAlert(false);
